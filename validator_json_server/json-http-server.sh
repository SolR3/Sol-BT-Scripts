...
sudo ufw allow $PORT1/tcp comment 'for serving the json file read by the huggingface page'

# Start HTTP servers with basic authentication on the specified ports
sudo python3 $1/json_http_server $PORT1 "$PATH_TO_BROWSE1" "$USERNAME" "$PASSWORD" &
PYTHON_PID=$!

# Trap to close the port and terminate the Python process when stopping the script
sudo trap 'kill -9 $PYTHON_PID 2>/dev/null; ufw delete allow $PORT1/tcp; exit 0' SIGINT SIGTERM
